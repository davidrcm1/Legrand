...
class RobotBackend:
    ...
    def start_plan(self, plan_name: str, speed: int):
        ...
        try:
            ...
            if not self.robot.operational():
                self._emit(BackendEvents.LOG, "[rdk] ENABLE ROBOT … (release E‑stop if needed)")
    ...
    def enable_gripper(self, name: str | None = None):
        """Enable the gripper via Device API. No helpers, just Init -> exist(name) -> Enable(name)."""
        if name is None:
            name = self.gripper_name

        # (Tiny safety: only proceed if connected and not moving/faulted)
        if not getattr(self, "connected", False) or getattr(self, "robot", None) is None:
        ...
        try:
            if self.robot.fault():
                ...
                return
            if self.robot.busy():
                self._emit(BackendEvents.LOG, "[rdk] enable_gripper(): robot is MOVING")
                self._emit(BackendEvents.RUNNING, True)
                return
        except Exception:
            # If bindings differ, still try device path
            pass

        try:
            import flexivrdk
            # Construct Device (most bindings accept the robot handle; if not, fall back)
            try:
                device = flexivrdk.Device(self.robot)
            except Exception:
                device = flexivrdk.Device()

            # Init()
            if hasattr(device, "Init"):
                device.Init()
            else:
                device.init()  # some bindings use lower-case

            # exist(name)?
            if hasattr(device, "exist"):
                exists = device.exist(name)
            elif hasattr(device, "Exists"):
                exists = device.Exists(name)
            else:
                self._emit(BackendEvents.LOG, "[rdk] Device.exist API not found")
                return

            if not bool(exists):
                self._emit(BackendEvents.LOG, f"[rdk] Device '{name}' not found")
                return
            ...
            if hasattr(device, "Enable"):
                ...
                device.enable(name)  # some bindings use lower-case

            self._emit(BackendEvents.LOG, f"[rdk] Gripper enabled: {name}")
