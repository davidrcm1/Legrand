...
class RobotBackend:
    ...
    def stop(self):
        if not self.connected or self.robot is None:
            ...
            return
        try:
            # If we might be paused, unpause first so Stop can take effect immediately
            try:
                if hasattr(self.robot, "PausePlan"):
                    self.robot.PausePlan(False)
            except Exception:
                pass

            # Issue Stop (twice, with a tiny delay) for immediate effect
            self.robot.Stop()
            try:
                time.sleep(0.05)
                self.robot.Stop()
            ...
            self._emit(BackendEvents.RUNNING, False)
            self._emit(BackendEvents.STOPPED, True)
            self._emit(BackendEvents.LOG, "[rdk] Stop requested (unpaused→stop)")
        except Exception as e:
