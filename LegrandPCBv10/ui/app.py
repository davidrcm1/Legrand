...
class App(ttk.Frame):
    def __init__(self, master):
        ...
        self.plan_name = tk.StringVar(
            value=PLAN_MAP.get(DEFAULT_JOB, DEFAULT_JOB)
        )
    ...
    def _build_ui(self):
        ...
        ttk.Label(self, text="Plan:").grid(row=3, column=2, sticky="e")
        ttk.Label(
        ...
        self.btn_seq = ttk.Button(btns, text="Run Sequencer", command=self.on_run_seq)
        self.btn_resetseq = ttk.Button(btns, text="Reset Seq1", command=self.on_reset_seq)

        for i, b in enumerate(
            (
                self.btn_start, self.btn_pause, self.btn_resume,
                self.btn_stop, self.btn_enable, self.btn_clearfault,
                self.btn_seq, self.btn_resetseq
            )
        ):
    ...
    def on_job_changed(self, *_):
        self.plan_name.set(self.plan_map.get(self.job.get(), self.job.get()))

    def on_start(self):
        # Single plan only (no sequencer)
        self.progress.set(0)
        ...
        threading.Thread(
            ...
            args=(self.plan_name.get(), self.speed.get()),
            daemon=True,
        ).start()
    ...
    def on_pause(self):
        ...
        self.btn_seq.configure(state="disabled")
        self.btn_resetseq.configure(state="disabled")
    ...
    def on_stop(self):
        # 1) Tell the sequencer to stop immediately and break out of waits
        if hasattr(self, "sequencer"):
            try:
                self.sequencer.stop()   # immediate: sets _stop_flag + events
            except Exception:
                pass

        # 2) Then stop the robot motion (now the worker won't start a new plan)
        try:
            self.backend.stop()
        except Exception:
            pass
        ...
        self.btn_resetseq.configure(state="normal")
    ...
    def on_run_seq(self):
        if not hasattr(self, "sequencer"):
            cfg = SequencerConfig(total_slots=6)
            self.sequencer = Sequencer(
                self.backend,
                cfg,
                log_cb=lambda m: self.event_q.put((BackendEvents.LOG, m)),
                speed_fn=self.speed.get,
            )
        self.sequencer.start_tray()
        # Reflect sequencer control state
        self.btn_pause.configure(state="normal")
        self.btn_stop.configure(state="normal")
        self.btn_start.configure(state="disabled")
        self.btn_resetseq.configure(state="disabled")

    def on_reset_seq(self):
        if hasattr(self, "sequencer"):
            self.sequencer.reset()
            # UI: after a successful reset we’re idle; Start remains usable
            self.btn_pause.configure(state="disabled")
            self.btn_resume.configure(state="disabled")
            self.btn_seq.configure(state="normal")

            # keep Stop enabled if you want an always-on E-stop
            self.event_q.put(
                (BackendEvents.LOG, "🔄 Sequencer reset to start (logical only)")
            )
        else:
            self.event_q.put((BackendEvents.LOG, "No sequencer yet to reset"))
            ...
            while True:
                ...
                if kind == BackendEvents.PROGRAM_REQUEST:
                    ...
                    if running:
                        ...
                        # Only disable if there is no active sequencer
                        if not (
                            hasattr(self, "sequencer")
                            and self.sequencer.get_state().get("running")
                        ):
                            self.btn_start.configure(state="normal")
                            self.btn_pause.configure(state="disabled")
                    ...
                    if is_fault:
                        ...
                        self.btn_stop.configure(state="normal" if self.running.get() else "disabled")
