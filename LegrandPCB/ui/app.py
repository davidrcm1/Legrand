...
from ..config.sequencer_profile import SEQUENCER_PROFILES
...
class App(ttk.Frame):
    def __init__(self, master):
        ...
        # Machine 1
        self.m1_commready  = tk.BooleanVar(value=False)
        self.m1_operating  = tk.BooleanVar(value=False)
        self.m1_loaded     = tk.BooleanVar(value=False)
        self.m1_awaiting   = tk.BooleanVar(value=False)
        self.m1_result     = tk.StringVar(value="-")

        # Machine 2
        self.m2_commready  = tk.BooleanVar(value=False)
        self.m2_operating  = tk.BooleanVar(value=False)
        self.m2_loaded     = tk.BooleanVar(value=False)
        self.m2_awaiting   = tk.BooleanVar(value=False)
        self.m2_result     = tk.StringVar(value="-")
    ...
    def _build_ui(self):
        ...
        # --- Machine 1 Tester Status ---
        m1_frame = ttk.Labelframe(self, text="Machine 1 Tester Status")
        m1_frame.grid(row=7, column=0, columnspan=6, sticky="we", pady=4)

        self._mk_indicator(m1_frame, "Comm Ready", self.m1_commready, 0)
        self._mk_indicator(m1_frame, "Operating", self.m1_operating, 1)
        self._mk_indicator(m1_frame, "Loaded", self.m1_loaded, 2)
        self._mk_indicator(m1_frame, "Awaiting Result", self.m1_awaiting, 3)

        ttk.Label(m1_frame, text="Result:").grid(row=0, column=4, sticky="e", padx=(12,2))
        ttk.Label(m1_frame, textvariable=self.m1_result, width=8).grid(row=0, column=5, sticky="w")

        # --- Machine 2 Tester Status ---
        m2_frame = ttk.Labelframe(self, text="Machine 2 Tester Status")
        m2_frame.grid(row=8, column=0, columnspan=6, sticky="we", pady=4)

        self._mk_indicator(m2_frame, "Comm Ready", self.m2_commready, 0)
        self._mk_indicator(m2_frame, "Operating", self.m2_operating, 1)
        self._mk_indicator(m2_frame, "Loaded", self.m2_loaded, 2)
        self._mk_indicator(m2_frame, "Awaiting Result", self.m2_awaiting, 3)

        ttk.Label(m2_frame, text="Result:").grid(row=0, column=4, sticky="e", padx=(12,2))
        ttk.Label(m2_frame, textvariable=self.m2_result, width=8).grid(row=0, column=5, sticky="w")

        # Progress + Log
        self.pbar = ttk.Progressbar(
            self, maximum=10, variable=self.progress, mode="determinate"
        )
        self.pbar.grid(row=9, column=0, columnspan=6, sticky="we", pady=6)

        logf = ttk.Labelframe(self, text="Log")
        logf.grid(row=10, column=0, columnspan=6, sticky="nsew", pady=6)
        ...
        statusf.grid(row=11, column=0, columnspan=6, sticky="nsew", pady=6)
    ...
    def on_start(self):
        ...
        # --- (re)create the Sequencer with the currently selected profile ---
        #     rebinding correct plans for set up
        selected = self.job.get()
        self.sequencer = Sequencer(
            backend=self.backend,
            config=SEQUENCER_PROFILES[selected],
            log_cb=self._log,
            speed_fn=self.speed.get,
        )
        # --- start the sequencer worker (one microplan at a time) ---
        self.sequencer.start_seq_worker()
        ...
        try:
            while True:
                ...
                if hasattr(self, "sequencer"):
                    ...
                    seq = self.sequencer
                    self.m1_commready.set(bool(getattr(seq, "tester_commready1", False)))
                    self.m1_operating.set(bool(getattr(seq, "tester_operating1", False)))
                    self.m1_loaded.set(bool(getattr(seq, "tester_loaded1", False)))
                    self.m1_awaiting.set(bool(getattr(seq, "awaiting_result1", False)))
                    r1 = getattr(seq, "tester_result1", None)
                    self.m1_result.set(str(r1).upper() if isinstance(r1, str) else "-")

                    self.m2_commready.set(bool(getattr(seq, "tester_commready2", False)))
                    self.m2_operating.set(bool(getattr(seq, "tester_operating2", False)))
                    self.m2_loaded.set(bool(getattr(seq, "tester_loaded2", False)))
                    self.m2_awaiting.set(bool(getattr(seq, "awaiting_result2", False)))
                    r2 = getattr(seq, "tester_result2", None)
                    self.m2_result.set(str(r2).upper() if isinstance(r2, str) else "-")
