...
from ..config.plans import PROJECT_TO_PLAN_NAME

class RobotBackend:
    """
    Flexiv backend.
    - connect(): establishes RDK session (no enable/motion yet).
    - monitor loop publishes FAULT/RUNNING and lightweight status logs.
    - other motion methods are shells to be implemented next.
    """
    ...
    def connect(self):
        try:
            ...
            self.connected = True
            self._emit(BackendEvents.LOG, f"[rdk] Connected to robot {sn}"
                                           + (f" via {ifaces}" if ifaces else ""))
    ...
    def _monitor(self):
        ...
        while not self._halt_mon.is_set():
            try:
                ...
                running = False
                try:
                    # If plan_info() returns a valid structure with a name, assume running
                    info = self.robot.plan_info()
                    if info and getattr(info, "pt_name", None):
                        running = True
                except Exception:
                    # plan_info might not be present or may throw; ignore
                    pass

                # Fallback: stopped() if available
                try:
                ...
                # 3) (Optional) Operational status breadcrumbs (comment out if too chatty)
                try:
                    status = self.robot.operational_status()
                    # Emit a sparse breadcrumb every second
                    # (Avoid spamming the log every cycle)
                    # We can throttle by sampling time or by change-detection later.
                    # For now, print every ~1s via time-based throttle:
    ...
    # ---- Shells to implement later ----
    def disconnect(self):
        # Stop monitor; if/when we add explicit RDK close calls, put them here.
        self._halt_mon.set()
        self._emit(BackendEvents.LOG, "[rdk] disconnect(): not implemented yet")
        
    def start_plan(self, plan_id: int, speed: int):
        """
        Follow Flexiv's tutorial sequence:
        1) Enable (if needed) and wait until operational
        2) Switch to NRT_PLAN_EXECUTION
        3) Execute a plan (index) and log plan_info while robot is busy()

        Notes:
        - UI passes a 1-based 'Project ID' from Elements; we convert to 0-based index.
        - If you prefer plan name execution later, swap to ExecutePlan(name, True).
        """
        if not self.connected or self.robot is None:
        ...
        try:
            # Clear fault if any (optional: mirror tutorial behavior)
            try:
                if self.robot.fault():
            ...
            # Switch to plan execution mode (per tutorial)
            try:
            ...
            # Optional: set velocity scaling (keep within 1..100)
            try:
            ...
            # Convert Elements Project ID (1-based) -> RDK index (0-based) when possible
            index = plan_id
            if plans and 1 <= plan_id <= len(plans):
                index = plan_id - 1

            # Execute the plan; allow it to continue even if our program exits (True)
            # Matches the tutorial: ExecutePlan(index, True)
            self.robot.ExecutePlan(index, True)
            name_hint = plans[index] if plans and 0 <= index < len(plans) else f"index {index}"
            self._emit(BackendEvents.LOG, f"[rdk] Executing plan: {name_hint}")
            self._emit(BackendEvents.RUNNING, True)

            # Print/emit plan info while running (tutorial uses while robot.busy())
            def _watch_busy():
                try:
                    while True:
                        if self.robot.fault():
                        ...
                        try:
                            ...
                            if pi:
                                self._emit(
                                    ...
                                    f"vel={getattr(pi,'velocity_scale',None)}, wait_step={getattr(pi,'waiting_for_step',None)}"
                                )
                        except Exception:
                            pass
                        time.sleep(1.0)
