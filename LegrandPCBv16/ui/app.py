...
class App(ttk.Frame):
    def __init__(self, master):
        ...
        self._halt_serial = threading.Event()
    ...
    def on_disconnect(self):
        self._halt_serial.set()  # stop serial loop
    ...
    def _start_serial_listener(self):
        if self._serial_th and self._serial_th.is_alive():
            return
        self._halt_serial.clear()

        def _worker():
            try:
                # Lazy import so Windows/Linux both work without blocking app start
                import serial
                from ..config.settings import SERIAL_PORT, SERIAL_BAUD
                self._log(f"[serial] opening {SERIAL_PORT} @ {SERIAL_BAUD}…")
                ser = serial.Serial(port=SERIAL_PORT, baudrate=SERIAL_BAUD, timeout=0.2)
                while not self._halt_serial.is_set():
                    try:
                    ...
                    if not line:
                        continue
                    msg = line.upper()
                    self._log(f"[serial] RX: {msg}")
                    # Tell the sequencer (PASS/FAIL/DONE etc.)
                    if hasattr(self, "sequencer"):
                        try:
                            self.sequencer.external_test_complete(msg)
                        except Exception as e:
                            self._log(f"[serial] notify error: {e}")
            except Exception as e:
                self._log(f"[serial] open error: {e}")

        self._serial_th = threading.Thread(target=_worker, daemon=True)
