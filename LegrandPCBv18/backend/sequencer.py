    ...
    def _worker(self):
        ...
        try:
            while not self._stop_flag:
                ...
                elif (self.tester_commready1 and ## use comm ready as a condition to action plans....
                    not progressed and
                    not self.tester_operating1 and
    ...
    def _serial_worker(self, machine_id: int):
        try:
            ...
            while halt_evt and not halt_evt.is_set():
                msg = None  # step 1: always define it upfront
                ...
                # step 3: decide if it's a valid verdict
                if "PASS" in up:
                    msg = "PASS"
                elif "FAIL" in up:
                    msg = "FAIL"
                ...
                # step 4: only act if message is valid
                if msg is not None:
