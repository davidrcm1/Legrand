...
class RobotBackend:
        ...
        while not self._halt_mon.is_set():
            try:
                ...
                try:
                    ...
                    if curr_fault:
                        try:
                            self.robot.SetDigitalOutputs({0: False, 1: False,2: True})   
                        except Exception as e:
                            self._emit(BackendEvents.LOG, f"[rdk] IO mirror error: {e}")
                ...
                try:
                    ...
                    # Red Indicator ("Fault")
                    if str(status) in ("OperationalStatus.NOT_ENABLED", "OperationalStatus.ESTOP_NOT_RELEASED"):
                        try:
                            self.robot.SetDigitalOutputs({0: False, 1: False, 2: True})  
                        except Exception as e:
                            self._emit(BackendEvents.LOG, f"[rdk] IO mirror error: {e}")
    ...
    def disconnect(self):
        ...
        try:
            if self.connected and self.robot is not None:
                ...
                try:
                    self.robot.SetDigitalOutputs({i: False for i in range(18)})  # only DO0–DO17
                    # or {i: False for i in range(18)} if you want to clear everything
                    self._emit(BackendEvents.LOG, "[rdk] All digital outputs set LOW on disconnect")
                except Exception as e:
                    self._emit(BackendEvents.LOG, f"[rdk] Failed to clear outputs: {e}")
        finally:
            self._halt_mon.set()
            try:
                if self._mon_th and self._mon_th.is_alive():
    ...
    def start_plan(self, plan_name: str, speed: int):
        ...
        try:
            ...
            try:
                ...
                # Output LED 
                try:
                    self.robot.SetDigitalOutputs({0: False, 1: True,2: False})  
                except Exception as e:
                    self._emit(BackendEvents.LOG, f"[rdk] IO mirror error: {e}")
    ...
    def pause(self):
        ...
        try:
            ...
            try:
                self.robot.SetDigitalOutputs({0: True, 1: False,2: False})   
            except Exception as e:
                self._emit(BackendEvents.LOG, f"[rdk] IO mirror error: {e}")
        except Exception as e:
